...
f=netcdf.netcdf_file("a_nnet_coef_matrix.nc")
l_hiddens = [CondenseLayer(l_in, num_units=300, nonlinearity=rectify,W=Constant(f.variables["W"][:]),b=Constant(f.variables["b"][:]))]
...
regs=[l2(l_out.W)]
#reg_logwt=create_param(Constant(0),(1,),name="reg_logwt")
#reg=(regs*T.exp(reg_logwt)).sum()
#etas=[lasagne.utils.shared_empty(p.ndim) for p in get_all_params(l_out)]
#theano.function([],[],updates=[(eta, lasagne.init.Normal().sample(c.get_value().shape)) for eta,c in zip(etas, get_all_params(l_out))])()
#create_param(GlorotUniform(), p.shape) for p in get_all_params(l_out)]
#def init_etas():
#    return [create_param(GlorotUniform(), p.shape) for p in get_all_params(l_out)]
#etas=theano.function([], [], updates=zip(eta)
#train_objective=train_loss+reg
#train_diff=T.sum([(T.grad(train_objective,p)*eta).sum() for p,eta in zip(get_all_params(l_out),etas)])
#constraints=[T.grad(train_objective-train_diff,reg_logwt)]
#constraints+=T.grad(valid_loss-train_diff,get_all_params(l_out))
#multipliers=[lasagne.utils.shared_empty(c.ndim) for c in constraints]
#objective=train_objective
#for (m,c) in zip(multipliers, constraints): objective+=(m*c).sum()
# this here is where the trouble is:can't even compute objective because of uninitialized stuff. need to give it the right dims somehow

#train_gradient=T.concatenate([T.grad(train_loss+reg,p).flatten() for p in get_all_params(l_out)])
#train_multiplier=create_param(Constant(0),(l_out.input_shape[1]*l_out.output_shape[1]+l_out.output_shape[1],))
#diff_gradient=T.concatenate([T.grad(train_loss-valid_loss,p).flatten() for p in get_all_params(l_out)])
#objective=train_loss+reg
#reg_objective=T.sum([T.grad(train_loss+reg,p).flatten().dot(T.grad(train_loss-valid_loss,p).flatten()).sum() for p in get_all_params(l_out)])
#((train_gradient.dot(diff_gradient))**2).sum()

updates=nesterov_momentum(objective,get_all_params(l_out),learning_rate=.01,momentum=.9)

update=theano.function([l_in.input_var,train_indices,valid_indices,target],[train_loss,valid_loss],updates=updates,allow_input_downcast=True)
#updates.update(nesterov_momentum(reg_objective**2,[reg_logwt],learning_rate=.01,momentum=.9))
update=theano.function([l_in.input_var,train_indices,valid_indices,target],[objective,train_loss,valid_loss,reg],allow_input_downcast=True)


# very cute but obvious problems with lack of sparsity.
